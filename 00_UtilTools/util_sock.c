...
// 客户端连接
int client_connect(const char* ip, const char* port, int sock_type, serv_sock_info_t* serv)
{
    // 1、创建指定传输类型的套接字
    serv->sock = socket(PF_INET, sock_type, 0);
    if (serv->sock == -1)  return 1001;
    ...
    if (sock_type == SOCK_STREAM) {
        int ret = connect(serv->sock, (struct sockaddr*)&serv->addr, serv->addr_len);
...
{
    int ret = client_connect(ip, port, SOCK_STREAM, serv);
    if (ret != 0)   return ret;
...
{
    int ret = client_connect(ip, port, SOCK_DGRAM, serv);
    if (ret != 0)   return ret;
